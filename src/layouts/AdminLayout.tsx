...
export const AdminLayout: React.FC<AdminLayoutProps> = ({ currentPath, onNavigate, children }) => {
  const { user, profile, activeArena, arenas, setActiveArena, signOut, isConfigured } = useAuth();
  ...
  const operationalMenuItems = [
    ...
    { name: 'Agenda', path: '/admin/agenda', icon: <Calendar className="w-4 h-4" /> },
    { name: 'Reservas', path: '/admin/reservas', icon: <BookmarkCheck className="w-4 h-4" /> },
    { name: 'Quadras', path: '/admin/quadras', icon: <Layers className="w-4 h-4" /> },
    { name: 'Modalidades', path: '/admin/modalidades', icon: <Volleyball className="w-4 h-4" /> },
    { name: 'Clientes', path: '/admin/clientes', icon: <Users className="w-4 h-4" /> },
    { name: 'Serviços', path: '/admin/servicos', icon: <Briefcase className="w-4 h-4" /> },
    { name: 'Financeiro', path: '/admin/financeiro', icon: <DollarSign className="w-4 h-4" /> },
    { name: 'Configurações', path: '/admin/configuracoes', icon: <Settings className="w-4 h-4" /> },
    { name: 'Usuários da Arena', path: '/admin/usuarios', icon: <UserCheck className="w-4 h-4" /> },
  ];

  const menuItems = profile?.role === 'SUPER_ADMIN'
    ? [
        { name: 'Gestão SaaS', path: '/admin/saas', icon: <ShieldCheck className="w-4 h-4" /> },
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col">
      ...
      <PersonaSwitcher onOpenTestSuite={() => setShowTestSuiteModal(true)} />

      {/* SQL Setup Modal */}
      <SqlSetupModal isOpen={showSqlModal} onClose={() => setShowSqlModal(false)} />

      {/* Integrity Test Suite Modal */}
      <IntegrityTestSuiteModal isOpen={showTestSuiteModal} onClose={() => setShowTestSuiteModal(false)} />
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="hidden lg:flex lg:flex-col lg:w-64 bg-slate-900/95 border-r border-slate-800 shrink-0">
          ...
          <div className="p-4 border-b border-slate-800">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-1.5">
                  <span className="font-extrabold text-base tracking-tight text-white">Arena<span className="text-emerald-400">Pro</span></span>
                  <span className="text-[10px] uppercase font-bold tracking-widest px-1.5 py-0.5 rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">SaaS</span>
                </div>
                <p className="text-[11px] text-slate-400 font-medium">Gestão Multi-Arena</p>
              </div>
            </div>

            {/* Tenant Selector (Active Arena) */}
            <div className="mt-4 relative">
              ...
              <button
                ...
                onClick={() => setArenaDropdownOpen(!arenaDropdownOpen)}
              ...
              >
                <div className="flex items-center gap-2 overflow-hidden">
                  ...
                  <span className="text-xs font-semibold text-slate-200 truncate">
                    {activeArena?.name || 'Selecione uma arena'}
                  </span>
                </div>
              ...
              {arenaDropdownOpen && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-slate-900 border border-slate-700 rounded-xl shadow-xl z-30 p-1.5 space-y-1 animate-fadeIn">
                  <div className="text-[10px] font-semibold text-slate-400 px-2 py-1">Alternar Arena (Multi-Tenant)</div>
                  {arenas.map((arena) => (
                    <button
                      key={arena.id}
                      onClick={() => {
                      ...
                      className={`w-full text-left px-2.5 py-2 rounded-lg text-xs font-medium transition flex items-center justify-between cursor-pointer ${
                        activeArena?.id === arena.id
                          ? 'bg-emerald-500/20 text-emerald-300 font-semibold'
                          : 'text-slate-300 hover:bg-slate-800'
                      }`}
                    >
                      <span className="truncate">{arena.name}</span>
                      {activeArena?.id === arena.id && (
          ...
          {/* Database Setup & Test Suite Buttons */}
          <div className="p-3 border-t border-slate-800 space-y-2">
            <button
              id="sidebar-test-suite-btn"
              onClick={() => setShowTestSuiteModal(true)}
              className="w-full flex items-center justify-between px-3 py-2 rounded-xl bg-teal-500/10 hover:bg-teal-500/20 border border-teal-500/30 text-xs font-medium text-teal-300 transition cursor-pointer"
            >
              <div className="flex items-center gap-2">
                <ShieldCheck className="w-3.5 h-3.5 text-teal-400" />
                <span>Auditoria de Integridade</span>
              </div>
              <span className="text-[10px] font-bold bg-teal-500/20 px-1.5 py-0.5 rounded text-teal-300">Executar</span>
            </button>

            <button
              id="open-db-schema-btn"
              onClick={() => setShowSqlModal(true)}
              className="w-full flex items-center justify-between px-3 py-2 rounded-xl bg-slate-950/60 hover:bg-slate-950 border border-slate-800 text-xs font-medium text-slate-300 hover:text-white transition cursor-pointer"
            >
              <div className="flex items-center gap-2">
            ...
            <div className="p-2 rounded-xl bg-slate-950/40 border border-slate-800/60 flex items-center justify-between">
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-xs font-bold text-slate-200 truncate">{profile?.full_name || 'Usuário'}</p>
                  <p className="text-[10px] text-emerald-400 font-medium">{getRoleLabel(profile?.role)}</p>
                </div>
              </div>
              <button
                id="admin-logout-btn"
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h1 className="text-base font-bold text-white tracking-tight">
                    {menuItems.find(m => m.path === currentPath)?.name || 'ArenaPro Gestão'}
                  </h1>
                ...
                <p className="text-xs text-slate-400 hidden sm:block">Painel administrativo e operacional</p>
              </div>
            </div>

            {/* Quick Actions & Client Mode Link */}
            <div className="flex items-center gap-2">
              <button
              ...
              <div className="w-72 bg-slate-900 border-r border-slate-800 flex flex-col h-full p-4 animate-slideRight">
                <div className="flex items-center justify-between pb-4 border-b border-slate-800">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-slate-100">ArenaPro</span>
                  </div>
                  <button
                ...
                <div className="py-3 border-b border-slate-800">
                  <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Arena Ativa</label>
                  <select
